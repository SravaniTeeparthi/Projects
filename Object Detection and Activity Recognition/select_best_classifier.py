...
class SampleDNN:
    ...
    def __init__(self,
        ...
        self.batch_normalization_   = batch_normalization
    ...
    def build(self):
        ...
        for i in range(self.num_conv_nets_ - 1):
            if self.structure_ =="S":
                print("Number of filter in SingleFB:",int(self.num_first_filters_))
                conv_layer = Conv3D(filters     = int(self.num_first_filters_),
                                    kernel_size = self.kernel_size_,
                                    activation  ='relu',
                                    data_format = "channels_last")(pool_layer)
            if self.structure_ =="P":
                print("Number of filter in PyramidFB:",int(self.num_first_filters_/2**(i+1)))
                conv_layer = Conv3D(filters     = int(self.num_first_filters_/2**(i+1)),
                                    kernel_size = self.kernel_size_,
                                    activation  ='relu',
                                    data_format = "channels_last")(pool_layer)
            if self.structure_ =="I":
                print("Number of filter in InverseFB:",int(self.num_first_filters_*2**(i+1)))
                conv_layer = Conv3D(filters     = int(self.num_first_filters_*2**(i+1)),
                                    kernel_size = self.kernel_size_,
                                    activation  ='relu',
                                    data_format = "channels_last")(pool_layer)
        ...
        if self.batch_normalization_ == "T":
        ...
        dense_layer1 = Dropout(self.dropout_)(dense_layer1)
...
def results(X_train_ori,y_train_ori,train_results,X_test,y_test,all_batches,all_epochs):
